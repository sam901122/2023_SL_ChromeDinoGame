...
class ListCatusAndBirds():
    ...
    def __init__( self ):
        ...
        for i in range( 0, 3 ):
            self.list.append(
                Catus( 500 + WINDOWWIDTH + random.randint( DISTANCE_MIN, DISTANCE_MAX ) * i, Y_CATUS,
                       random.randint( 0, 3 ) ) )
    ...
    def update( self, score ):
        ...
        if self.list[ 0 ].x < -132:
            ...
            if self.speed > SPEED_GROUND * 1.5:
                ...
                if rand == 5:
                    self.list.append(
                        Bird( self.list[ 1 ].x + random.randint( DISTANCE_MIN + 200, DISTANCE_MAX + 100 ),
                ...
                self.list.append(
                    Catus( self.list[ 1 ].x + random.randint( DISTANCE_MIN, DISTANCE_MAX ), Y_CATUS,
                           random.randint( 0, 3 ) ) )
    ...
    while True:
        ...
        if isCollision( tRex, ls ):
            ...
            while True:
                ...
                ls.draw()
                score.draw()
                DISPLAYSURF.blit( gameOverTextSurface,
                                  ( int( WINDOWWIDTH/2 - gameOverTextSurface.get_width() / 2 ), 50 ) )
                blinkText.update()
                blinkText.draw()
                pygame.display.update()
                FPSCLOCK.tick( FPS )
